...
st.set_page_config(page_title="Cofractionation - Eukaryoma PPI", page_icon="🧬", layout="wide")
st.title("Cofractionation")
st.write(
    "Protein pairs ranked by cofractionation correlation score. Other sources' scores are shown when "
...
SCORE_COL = "cofractionation_score"


@st.cache_data
def get_cofractionation_pairs():
...
scored_pairs = get_cofractionation_pairs()
if scored_pairs.empty:
    st.info("No cofractionation data available.")
    st.stop()

st.caption(f"{len(scored_pairs):,} pairs have a cofractionation score.")
...
ui.render_ranked_pair_table(
    ...
    column_config={
        SCORE_COL: st.column_config.NumberColumn("Cofractionation score", format="%.3f"),
    ...
    score_fields=[(SCORE_COL, "Cofractionation score")]
